...
class _ReadingQuizScreenState extends State<ReadingQuizScreen> {
  final ReadingQuizRepository _repository = ReadingQuizRepository();
  List<Word> quizWords = [];
  List<Word> dueWords = [];
  ...
  Future<void> _loadQuizData() async {
    try {
      ...
      final loadedWords = await _repository.getReadingQuizWords(widget.readingId);
      final loadedDueWords = await _repository.getReadingDueWords(widget.readingId);
      final stats = await _repository.getReadingQuizStats(widget.readingId);
      ...
      print('Error loading reading quiz data: $e');
      setState(() {
        isLoading = false;
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.readingTitle),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Hãy thêm từ vào danh sách ôn tập từ bài reading này',
                            style: TextStyle(
                              fontSize: 14,
  ...
  void _startQuiz() {
    if (dueWords.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Không có từ nào cần ôn tập ngay bây giờ'),
    ...
    if (mounted) {
      showDialog(
        ...
        builder: (context) => QuizPlanningDialog(
          words: dueWords,
          title: 'Quiz: ${widget.readingTitle}',
              ...
              MaterialPageRoute(
                builder: (context) => QuizGameScreen(
                  ...
                  onWordProgressUpdate: (word, isCorrect) async {
                    await _repository.updateReadingWordProgress(widget.readingId, word, isCorrect);
                  },
                ),
              ),
            ).then((_) {
  ...
  void _removeWordFromQuiz(Word word) async {
    ...
    if (confirm == true) {
      try {
        final success = await _repository.removeWordFromReadingQuiz(widget.readingId, word);
        if (success) {
